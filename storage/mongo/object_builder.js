...
let objectBuilder = {
    create: catchWrapDbObjectBuilder(`${NAMESPACE}.create`, async (req) => {
        ...
        try {
            ...
            let {
                payload, data, appendMany2ManyObjects
            ...
            if (tableData && tableData.is_login_table && !data.from_auth_service && !req.blocked_login_table) {
                ...
                if (tableAttributes && tableAttributes.auth_info) {
                    ...
                    if (!data[authInfo['client_type_id']] || !data[authInfo['role_id']] || !(data[authInfo['login']] || data[authInfo['email']] || data[authInfo['phone']])) {
                        throw new Error('This table is auth table. Auth information not fully given')
                    }

                    let loginTable = await allTableInfos['client_type']?.models?.findOne({
                        guid: data[authInfo['client_type_id']], table_slug: tableData.slug
                    })
                    ...
                    if (loginTable) {
                        ...
                        await grpcClient.createUserAuth(authCheckRequest)
                            ...
                            .then((res) => {
                                tableInfo.models.updateOne({guid: ownGuid}, {
                                    $set: {
                                        email: (authCheckRequest.email), user_id_auth: res.user_id,
                                    }
                                }).catch((err) => {
            ...
            if (req.table_slug === 'person') {
                if (!data['client_type_id'] || !data['role_id'] || !data['login'] || !data['phone_number'] || !data['email'] || !data['password']) {
            ...
            if (tableData) {
                const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                    code: 201, table_id: tableData.id, action_type: "CREATE"
        ...
        try {
            ...
            try {
                ...
                const isLoginTable = (!data?.from_auth_service && tableModel && tableModel?.is_login_table && response && tableAttributes && tableAttributes?.auth_info && authInfo);
                    ...
                    if (authInfo['password'] && data[authInfo['password']] !== "") {
                        if (String(data[authInfo['password']]).length !== 60) {
                            ...
                            const loginTable = allTableInfo['client_type']?.models?.findOne({
                                guid: response[authInfo['client_type_id']], table_slug: tableModel.slug
                            });
            ...
            let {
                data, appendMany2Many, deleteMany2Many
            ...
            if (String(personTableRequest?.user_id_auth).length !== 0) {
                await allTableInfo["person"]?.models.findOneAndUpdate({guid: personTableRequest?.guid}, {$set: personTableRequest}, {upsert: true})
            }
        ...
        const relations = await Relation.find({
            table_from: req.table_slug, type: "One2One"
        })
        ...
        for (const relation of relationsM2M) {
            ...
            relationQueries.push({
                slug: relation.field_from, relation_id: relation.id
            })
        }
        if (relationQueries.length > 0) {
            const fields = await Field.find({$or: relationQueries})
        ...
        let {
            fieldsWithPermissions, unusedFieldsSlugs
        ...
        let output = await tableInfo.models.findOne(filter, {
            created_at: 0, updated_at: 0, createdAt: 0, updatedAt: 0, _id: 0, __v: 0, ...unusedFieldsSlugs
        ...
        for (const field of tableInfo.fields) {
            ...
            if (field.type === "FORMULA") {
                if (attributes.table_from && attributes.sum_field) {
                    attribute_table_from_slugs.push(attributes.table_from.split("#")[0]);
                    attribute_table_from_relation_ids.push(attributes.table_from.split("#")[1]);
                }
            }
        }
        const relationFieldTables = await tableVersion(mongoConn, {
            slug: {$in: attribute_table_from_slugs}, deleted_at: "1970-01-01T18:00:00.000+00:00",
        }, data.version_id, false);
        ...
        const relationFields = await Field.find({
            relation_id: {$in: attribute_table_from_relation_ids}, table_id: {$in: relationFieldTableIds},
        });
        ...
        if (isChanged) {
            await objectBuilder.update({
                table_slug: req.table_slug, project_id: req.project_id, data: struct.encode(output)
            })
        }
        if (output) {
            ...
            if (element.attributes && !(element.type === "LOOKUP" || element.type === "LOOKUPS")) {
                ...
                for (const field of tableElementFields) {
                    if (field.autofill_field && field.autofill_table && field.autofill_table === relationTableSlug) {
                        let autofill = {
                            field_from: field.autofill_field, field_to: field.slug,
                        }
                        autofillFields.push(autofill)
                    }
                }
                elementField.attributes["autofill"] = autofillFields, decodedFields.push(elementField)
            }
        }
        ;
            ...
            if (field.type === "LOOKUP" || field.type === "LOOKUPS") {
                ...
                if (relation) {
                    let viewFieldsResp = await Field.find({
                        ...
                        createdAt: 0, updatedAt: 0, created_at: 0, updated_at: 0, _id: 0, __v: 0
                    })
        ...
        if (table) {
            const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                code: 200, table_id: table.id, action_type: "GET_SINGLE",
            })
            if (customErrMsg) {
                customMessage = customErrMsg.message
            }
        ...
        return {
            table_slug: data.table_slug, data: struct.encode({
                response: output, fields: decodedFields
            ...
            if (relations.length == 0) {
                        ...
                        if (typeof params[key] === "object") {
                            ...
                            if (tableParams[key.split('.')[0]]) {
                                ...
                                tableParams[key.split('.')[0]] = {
                                    [key.split('.')[1]]: interval, select: '-_id'
                                }
                            }
                            ...
                            if (tableParams[key.split('.')[0]]) {
                                ...
                                tableParams[key.split('.')[0]] = {
                                    [key.split('.')[1]]: {$regex: params[key]}, select: '-_id'
                                }
                            }
                        }
                    }
                ...
                for (const relation of relations) {
                    ...
                    if (tableParams[table_to_slug]) {
                        papulateTable = {
                            path: table_to_slug, match: tableParams[table_to_slug], populate: deepRelations,
                        }
                    } else {
                        if (relation.type === "Many2Dynamic") {
                            for (dynamic_table of relation.dynamic_tables) {
                        ...
                        papulateTable = {
                            path: table_to_slug, populate: deepRelations
                        }
                    }
                    populateArr.push(papulateTable)
                }
        ...
        if (tableWithVersion) {
            const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                code: 200, table_id: tableWithVersion.id, action_type: "GET_LIST_SLIM"
            })
            if (customErrMsg) {
                customMessage = customErrMsg.message
            }
            ...
            for (const field of formulaFields) {
                ...
                if (field.type === "FORMULA") {
                    if (attributes.table_from && attributes.sum_field) {
                        ...
                        let matchParams = {
                            [matchField]: {'$eq': res.guid}, ...filters
                        }
        ...
        if (updatedObjects.length) {
            await objectBuilder.multipleUpdateV2({
                table_slug: req.table_slug, project_id: req.project_id, data: struct.encode({objects: updatedObjects})
            })
        }
        ...
        return {
            table_slug: req.table_slug, data: response, is_cached: tableResp.is_cached, custom_message: customMessage
        }
        ...
        if (params.with_relations) {
            if (params.selected_relations && params.selected_relations.length) {
                relations = await Relation.find({
                    $or: [{
                        table_from: req.table_slug, table_to: {$in: params.selected_relations},
                    }, {
                        "dynamic_tables.table_slug": req.table_slug
                    }]
                })
        ...
        if (params.$or) {
            params.$and = [{$or: params.$or}, {
                $or: [{deleted_at: new Date("1970-01-01T18:00:00.000+00:00")}, {deleted_at: null}]
            }]

            delete params.$or
        } else {
            params.$or = [{deleted_at: new Date("1970-01-01T18:00:00.000+00:00")}, {deleted_at: null}]
        }
        order = {...order, _id: 1}
        if (limit !== 0) {
            if (relations.length == 0) {
        ...
        const permission = await permissionTable.models.findOne({
            $and: [{role_id: params["role_id_from_token"]}, {table_slug: req.table_slug}]
        })
        const relations = await Relation.find({
            $or: [{table_from: req.table_slug}, {table_to: req.table_slug}, {"dynamic_tables.table_slug": req.table_slug}]
        })

        if (permission?.is_have_condition) {
            const automatic_filters = await automaticFilterTable.models.find({
                $and: [{role_id: params["role_id_from_token"]}, {table_slug: req.table_slug}, {method: "read"}]
            })
            if (automatic_filters.length) {
        ...
        for (let view of views) {
            const permission = await viewPermission.models.findOne({
                view_id: view.id, role_id: params.role_id_from_token
            }).lean() || {}
        ...
        if (with_relations) {
            ...
            if (relation_table_to_slugs.length > 0) {
                let relationTables = await tableVersion(mongoConn, {slug: {$in: relation_table_to_slugs}}, params.version_id, false);
            ...
            if (relation_table_to_slugs.length > 0 && relationFieldSlugsR.length > 0) {
                const childRelations = await Relation.find({
                    table_from: {$in: relation_table_to_slugs}, table_to: {$in: relationFieldSlugsR},
                });
            ...
            if (relationFieldSlugsR.length > 0) {
                const childRelationTables = await tableVersion(mongoConn, {slug: {$in: relationFieldSlugsR}}, params.version_id, false);
        ...
        if (limit !== 0) {
            if (relations.length == 0) {
                result = await tableInfo.models.find({$and: [params]}, {
                    createdAt: 0, updatedAt: 0, created_at: 0, updated_at: 0, _id: 0, __v: 0, ...unusedFieldsSlugs
                }, {sort: order}).skip(offset)
                    .limit(limit)
                    .lean();
                count = await tableInfo.models.countDocuments(params);
        ...
        const response = struct.encode({
            count: count, response: result, fields: decodedFields, views: views, relation_fields: relationsFields,
        });
        const tableWithVersion = await tableVersion(mongoConn, {slug: req.table_slug})
        let customMessage = ""
        if (tableWithVersion) {
            const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                code: 200, table_id: tableWithVersion.id, action_type: "GET_LIST"
            })
            if (customErrMsg) {
                customMessage = customErrMsg.message
            }
        ...
        if (limit !== 0) {
            if (relations.length == 0) {
                ...
                let query = tableInfo.models.find({...params}, {
                    createdAt: 0, updatedAt: 0, created_at: 0, updated_at: 0, _id: 0, __v: 0, ...unusedFieldsSlugs
                }).sort(order);
        ...
        for (const field of tableInfo.fields) {
            ...
            if (field.type === "FORMULA") {
                if (attributes.table_from && attributes.sum_field) {
                    ...
                    const relationFieldTable = await table.findOne({
                        slug: attributes.table_from.split('#')[0], deleted_at: "1970-01-01T18:00:00.000+00:00"
                    })
                    const relationField = await Field.findOne({
                        relation_id: attributes.table_from.split('#')[1], table_id: relationFieldTable.id
                    })
                    if (!relationField || !relationFieldTable) {
                        output[field.slug] = 0
                        continue
        ...
        if (tableWithVersion) {
            const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                code: 200, table_id: tableWithVersion.id
            })
            if (customErrMsg) {
                customMessage = customErrMsg.message
            }
        ...
        try {
            ...
            const tableModel = await tableVersion(mongoConn, {
                slug: req.table_slug, deleted_at: new Date("1970-01-01T18:00:00.000+00:00")
        ...
        try {
            ...
            if (tableWithVersion) {
                const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                    code: 200, table_id: tableWithVersion.id, action_type: "GET_LIST_IN_EXCEL"
                })
                if (customErrMsg) {
                    customMessage = customErrMsg.message
                }
        ...
        try {
            ...
            if (tableWithVersion) {
                const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                    code: 200, table_id: tableWithVersion.id, action_type: "DELETE_MANY2MANY"
                })
                if (customErrMsg) {
                    customMessage = customErrMsg.message
                }
        ...
        try {
            ...
            if (tableWithVersion) {
                const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                    code: 200, table_id: tableWithVersion.id, action_type: "APPEND_MANY2MANY",
                })
                if (customErrMsg) {
                    customMessage = customErrMsg.message
                }
        ...
        const customEvents = await CustomEvent.find({
            ...
            created_at: 0, updated_at: 0, createdAt: 0, updatedAt: 0, _id: 0, __v: 0,
        }, {
            sort: {created_at: -1},
        }).populate({
            path: "functions", select: "-created_at -updated_at -createdAt -updatedAt -_id -__v",
        }).lean();

        let customEventWithPermission = await AddPermission.toCustomEvent2(customEvents, params?.role_id_from_token, req.table_slug, req.project_id);

        const table = await Table.findOne({slug: req.table_slug})
        const returnedTable = {
            id: table.id,
            slug: table.slug,
            label: table.label,
        ...
        try {
            ...
            for (const object of data.objects) {
                ...
                if (objectFromTable) {
                    ...
                    let requestToUpdate = {
                        table_slug: req.table_slug, project_id: req.project_id, data: struct.encode(object)
                    }
            ...
            if (tableWithVersion) {
                const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                    code: 200, table_id: tableWithVersion.id, action_type: "MULTIPLE_UPDATE"
                })
                if (customErrMsg) {
                    customMessage = customErrMsg.message
                }
        ...
        try {
            ...
            if (data.objects.length !== 0) {
                for (const object of data.objects) {
                    ...
                    let request = {
                        ...
                        env_id: req.env_id,
                        company_project_id: req.company_project_id,
                    }
                    if (!object.is_new) {
            ...
            if (req.blocked_builder) {
                return {
                    table_slug: data.table_slug, data: struct.encode({objects: response}), custom_message: ""
                };
            }
        ...
        try {
            ...
            if (tableData && tableData.is_login_table && !data.from_auth_service) {
                ...
                if (tableAttributes && tableAttributes.auth_info) {
                    authInfo = tableAttributes.auth_info
                    let loginTable = await allTableInfos['client_type']?.models?.findOne({
                        guid: objects[0][authInfo['client_type_id']], table_slug: tableData.slug
                    })
                    if (loginTable) {
                        isLoginTable = true
                        if (authInfo['login_strategy'] && authInfo['login_strategy'].length) {
            ...
            for (const object of data.objects) {
                ...
                let request = {
                    data: struct.encode(object), table_slug: req.table_slug, project_id: req.project_id
                }
                let {
                    payload, data, appendMany2ManyObjects
            ...
            if (responseFromAuth && responseFromAuth?.user_ids && responseFromAuth?.user_ids?.length === guids.length) {
                for (let i = 0; i < guids.length; i++) {
                    bulkWriteGuids.push({
                        updateOne: {
                            filter: {guid: el}, update: {$set: {user_id_auth: responseFromAuth.user_ids[i]}}
                        }
                    })
                }
            }
        ...
        try {
            ...
            for (const obj of datas.objects) {
                let request = {
                    data: struct.encode(obj), table_slug: req.table_slug, project_id: req.project_id,
                }

                let {
                    data, event, appendMany2Many, deleteMany2Many
                } = await prepareFunction.prepareToUpdateInObjectBuilder(request, mongoConn)

                let bulk = {
                    updateOne: {
                        filter: {guid: data.id}, update: data
                    }
                }
                for (const resAppendM2M of appendMany2Many) {
        ...
        try {
            ...
            for (const obj of datas.objects) {
                ...
                let bulk = {
                    updateOne: {
                        filter: {[fieldsSlug]: data[fieldsSlug]}, update: {
        ...
        try {
            ...
            let resp = await objectBuilder.getList({
                project_id: req.project_id, table_slug: req.table_slug, data: struct.encode(request)
            })
            ...
            if (view && view.attributes?.balance && view.attributes.balance?.table_slug && view.attributes.balance?.field_id) {
                ...
                for (let acc of accounts) {
                    map_total_accounts[acc.guid] = {
                        name: acc.name, total: acc[number_field.slug]
                    }
                ...
                for (let key in map_total_accounts) {
                    items.push({
                        name: map_total_accounts[key].name, id: key
                    })
                }
                            ...
                            if (account) {
                                if (!account.amounts) {
                                    ...
                                    account.amounts.push({
                                        month: key, amount: map_total_accounts[keyJ]['total'] - count
                                    })
                                }
                            }
                        }
                ...
                for (const date of dates) {
                            ...
                            if (resultOption.length) {
                                if (option.type === "debet") {
                                    if (monthlyAmount) {
                                        ...
                                        monthlyAmount = {
                                            amount: resultOption[0].res, month: key
                                        }
                                    }
                    ...
                    while (parentObj) {
                        ...
                        if (parentMonthlyAmount && monthlyAmount) {
                            ...
                            parentMonthlyAmount = {
                                amount: monthlyAmount.amount, month: monthlyAmount.month
                            }
            ...
            if (tableWithVersion) {
                const customErrMsg = await mongoConn?.models["CustomErrorMessage"]?.findOne({
                    code: 200, table_id: tableWithVersion.id, action_type: "GET_FINANCIAL_ANALYTICS"
                })
                if (customErrMsg) {
                    customMessage = customErrMsg.message
                }
        ...
        if (Object.keys(params.rows).length) {
            let row_data = await allTables[req.table_slug].models.aggregate([{...params.rows.match}, {...params.rows.group}])
            ...
            responseRow = await rowTableInfo.models.aggregate([{$match: {guid: {$in: row_ids}}}, {$addFields: {table_slug: params.rows.row_table_slug + "_id"}}, ...params.rows.lookups, {$project: {...params.rows.project}}, {$sort: {...params.rows.sort}}])
            ...
            responseRow = await relationTableInfo.models.aggregate([{...rows_inside_relation_match_query}, {$addFields: {table_slug: params.rows_inside_relation.row_relation_table_slug + "_id"}}, ...params.rows_inside_relation.lookups, {$project: {...params.rows_inside_relation.project}}, {$sort: {...params.rows_inside_relation.sort}}])
            ...
            if (rowRelationTableData.length > 0) {
                ...
                responseRow = await rowRelationNestedTableInfo.models.aggregate([{$match: {guid: {$in: row_relation_ids}}}, {$addFields: {table_slug: params.rows_relation_nested.row_relation_nested_table_slug + "_id"}}, {$project: {...params.rows_relation_nested.project}}, {$sort: {...params.rows_relation_nested.sort}}])
                ...
                const rowRelationNestedTableInfo = allTables[params.rows_relation_nested.row_relation_nested_table_slug]
                responseRow = await rowRelationNestedTableInfo.models.aggregate([{$match: {guid: {$in: row_inside_relation_ids}}}, {$addFields: {table_slug: params.rows_relation_nested.row_relation_nested_table_slug + "_id"}}, {$project: {...params.rows_relation_nested.project}}, {$sort: {...params.rows_relation_nested.sort}}])
            }
        }

        // Columns...
        if (Object.keys(params.columns).length) {
            let column_data = await allTables[req.table_slug].models.aggregate([{...params.columns.match}, {...params.columns.group}])
            ...
            responseColumn = await columnTableInfo.models.aggregate([{$match: {guid: {$in: column_ids}}}, {$project: {...params.columns.project}}, {$sort: {...params.columns.sort}}])
        }
        ...
        if (Object.keys(params.values).length) {
            for (let [key, value] of Object.entries(params.values)) {
                ...
                let vals = await keyTableInfo.models.aggregate([{$addFields: {...dateFromStringQuery}}, {$sort: {...sortValuesQuery}}, {$match: {...matchQuery}}, {...value.group}, {$project: {_id: 0}}])
        ...
        if (Object.keys(params.total_values).length) {
            for (let [key, value] of Object.entries(params.total_values)) {
                ...
                const keyTableInfo = allTables[key]
                let total_vals = await keyTableInfo.models.aggregate([{$addFields: {...dateFromStringQuery}}, {$match: {...matchQuery}}, {...value.group}, {$project: {_id: 0}}])
        ...
        if (params.match) {
            if (params.date_filter) {
                ...
                params.match.$match = {
                    ...params.match.$match, [params.date_filter.match_date_field]: {
                    ...
                    if (tableAttributes && tableAttributes.auth_info) {
                        ...
                        for (const obj of response) {
                            if (tableModel && !data.from_auth_service) {
                                ...
                                const loginTable = allTableInfo['client_type']?.models?.findOne({
                                    guid: obj[authInfo['client_type_id']], table_slug: tableModel.slug
                                })
        ...
        const relations = await Relation.find({
            $or: [{
                table_from: req.table_slug, type: 'Many2One'
            }]
        })
        ...
        for (const relation of relations) {
            ...
            lookups.push({
                $lookup: {
                    from: from, localField: field.slug, foreignField: "guid", as: table_to_slug
                }
            })
            lookUpFor.push({
                $lookup: {
                    from: from, localField: "_id." + field.slug, foreignField: "guid", as: table_to_slug
                }
            })
        }
                ...
                if (params.view_type == "TIMELINE") {
                    groupBy["data"] = {
                        $push: {
                            "guid": "$guid",
                            "createdAt": "$createdAt",
                            [view.attributes.visible_field]: "$" + view.attributes.visible_field, ...projectColumnsWithDollorSign, ...numberfieldWithDollorSign, ...lookupAddFields, ...lookupGroupField,
                        }
                    };
                } else {
                    groupBy["data"] = {
                        $push: {
                            "guid": "$guid",
                            "createdAt": "$createdAt", ...projectColumnsWithDollorSign, ...numberfieldWithDollorSign, ...lookupAddFields, ...sumFieldWithDollorSign
                        }
                    };
                }
            }
        ...
        const controlKeys = new Set(["client_type_id_from_token", "builder_service_view_id", "user_id_from_token", "role_id_from_token", "language_setting", "with_relations", "row_view_id", "view_type", "tables", "offset", "search", "order", "limit", "lte", "gte",])
        let matchFilters = {}
        ...
        if (typeOfLastLabel == "LOOKUP" && secTitleField != "") {
            aggregationPipeline.push({
                $lookup: {
                    from: groupRelation, localField: "_id", foreignField: "guid", as: secTitleField + "_data"
                }
            })
        } else if (typeOfLastLabel == "LOOKUP") {
            aggregationPipeline.push({
                $lookup: {
                    from: groupRelation, localField: "_id", foreignField: "guid", as: titleField + "_data"
                }
            })
        }
        ...
        if (params.view_type == "TABLE") {
            aggregationPipeline.push({
                '$addFields': {
                    [titleField]: '$_id', 'group_by_type': typeOfLastLabel, 'group_by_slug': groupBySlug, 'createdAt': {
                        "$arrayElemAt": ["$data.createdAt", 0]
                    }
                },
            }, {"$sort": order})
        ...
        try {
            ...
            await RelationStorage.CopyRelations({
                project_id: req.project_id, relations: relations_from, views: relation_views_from
            })
            ...
            for (const view of data.views) {
                ...
                for (const role of roles) {
                    view_permission.push({
                        guid: v4(), view: true, edit: true, delete: true, role_id: role.guid, view_id: view.id
                    })
                }
            }
        ...
        try {
            ...
            let pipeline = [{$match: filter}, {
                $lookup: {
                    from: pluralize(req.table_slug), let: {currentGuid: "$guid"}, pipeline: [{
                        $match: {
                            $expr: {$eq: [`$${req.table_slug}_id`, "$$currentGuid"]}
                        }
                    }, {$limit: 1}, {$project: {_id: 1}}], as: "children"
                }
            }, {
                $graphLookup: {
            ...
            }, {$unwind: {path: "$ancestors", preserveNullAndEmptyArrays: true}}, {$sort: {"ancestors.depth": 1}}];
            ...
            const projectStage = {
                $project: {
                    _id: 0, path: {
                        $concatArrays: [{
                            $map: {
                                input: {$reverseArray: "$ancestors"}, as: "a", in: "$$a.guid"
                            }
                        }, ["$guid"]]
                    }, has_child: 1
                }
        ...
        try {
            ...
            const {
                group_by: groupBy = {}, subgroup_by: subgroupBy = {}
            } = params;
            ...
            const filterFields = Object.keys(params).filter(key => !['fields', 'group_by', 'subgroup_by', 'limit', 'offset', 'tables', 'view_fields', 'search'].includes(key));
            ...
            const groupPipeline = [{$match: matchConditions}, {
                $unwind: {
                    path: `$${groupByField}`, preserveNullAndEmptyArrays: true
                }
            }, {
                $group: {
                    _id: `$${groupByField}`, count: {$sum: 1}
                }
            }, {
                $project: {
                    _id: 0, name: {$ifNull: ["$_id", noGroupValue]}, count: 1
                }
            }];
            ...
            if (hasSubgroup) {
                const subgroupPipeline = [{$match: matchConditions}, {
                    $addFields: {
                        _subgroupField: {
                            $cond: {
                    ...
                    $unwind: {
                        path: "$_subgroupField", preserveNullAndEmptyArrays: true
                    }
                }, {
                    $group: {
                        _id: {$ifNull: ["$_subgroupField", noGroupValue]}, count: {$sum: 1}
                    }
                }, {
                    $project: {
                ...
                subgroups = subgroupsResult.map(subgroup => ({
                    name: subgroup.name, count: subgroup.count
                }));
            }
        ...
        try {
            ...
            pipeline.push({
                $facet: {
                    data: [{$sort: {[orderBy]: 1, board_order: 1, updatedAt: -1}}, {$skip: offset}, {$limit: limit}, {
                        $project: {
                            _id: 0, ...fields.reduce((acc, field) => ({
                                ...acc, [field]: 1
            ...
            pipeline.push({
                $project: {
                    data: 1, count: {$arrayElemAt: ["$totalCount.count", 0]}
                }
            });
            ...
            const groupedData = groupResults(rows, {
                groupByField, subgroupByField, hasSubgroup, noGroupValue
            });
            ...
            return {
                table_slug: req.table_slug, project_id: req.project_id, data: struct.encode({
                    response: groupedData, count
                })
            };
        } catch (err) {
            throw err;
