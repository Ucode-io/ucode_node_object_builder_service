...
let objectBuilder = {
    create: catchWrapDbObjectBuilder(`${NAMESPACE}.create`, async (req) => {
        ...
        try {
            ...
            const tableInfo = (await ObjectBuilder(false, req.project_id))[req.table_slug]
            ...
            for (const field of tableInfo.fields) {
                if (field.type === "FORMULA") {
                    ...
                    if (attributes.table_from && attributes.sum_field) {
                        ...
                        const resultFormula = await (await ObjectBuilder(false, req.project_id))[attributes.table_from].models.aggregate(pipelines)
    ...
    getList: catchWrapDbObjectBuilder(`${NAMESPACE}.getList`, async (req) => {
        try {
            ...
            if (permission?.is_have_condition) {
                const automaticFilterTable = (await ObjectBuilder(false, req.project_id))["automatic_filter"]
            ...
            console.log(`[2] seconds elapsed = ${Math.floor(millis / 1000)}`);
            if (clientTypeId) {
                const clientTypeTable = (await ObjectBuilder(false, req.project_id))["client_type"]
    ...
    deleteManyToMany: catchWrapDbObjectBuilder(`${NAMESPACE}.deleteManyToMany`, async (data) => {
        try {
            const fromTableModel = (await ObjectBuilder(false, data.project_id))[data.table_from]
            const toTableModel = (await ObjectBuilder(false, data.project_id))[data.table_to]
                ...
                for (const date of dates) {
                    ...
                    if (chartOfAccount && chartOfAccount.options && chartOfAccount.options.length) {
                        for (const option of chartOfAccount.options) {
                            ...
                            const resultOption = await (await ObjectBuilder(false, req.project_id))[option.table_slug].models.aggregate(pipelines)
